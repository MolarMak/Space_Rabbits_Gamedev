...
import models.v2.{LoginRequest, LoginResponse, ProfileResponse, RegisterRequest, RegisterResponse}
import org.scalatest.concurrent.ScalaFutures
import org.scalatest.{Matchers, WordSpec}
import repositories.{FactRepository, StatisticRepository, UserRepository}
import slick.jdbc.PostgresProfile.api._

class ProfileV2Spec extends WordSpec with Matchers with ScalatestRouteTest with ScalaFutures {
  ...
  "first user statistic is fully 0" in {
    ...
    def getUserStatistic(token: String) = {
      Get(s"/api/$apiVersion/loadScores?readId=0") ~> RawHeader("Authorization", token) ~> Route.seal(quizApi.loadScores) ~> check {
        status shouldEqual StatusCodes.OK
  ...
  "read 10 facts offline and get statistic" in {
    ...
    def getUserStatistic(token: String) = {
      Get(s"/api/$apiVersion/loadScores?readId=10") ~> RawHeader("Authorization", token) ~> Route.seal(quizApi.loadScores) ~> check {
        status shouldEqual StatusCodes.OK
        decode[ProfileResponse](responseAs[String]) match {
          case Right(json) =>
            json.result shouldEqual true
